...
```mermaid
flowchart LR
    A[랜딩: 오늘의 타로 뽑기] --> B[카드 선택 연출<br>22장 중 1장]
    B --> C[결과 페이지<br>카드 해석 + 추천 원석]
...
### F1. 타로 뽑기 (P0)
- 메이저 아르카나 **22장, 1장 뽑기** 방식 (78장 스프레드는 P2).
- 뒷면 카드 배열에서 사용자가 직접 1장을 탭하여 선택 (랜덤은 클라이언트에서 결정).
...
| 78장 풀덱·역방향·3카드 스프레드 | 22장 반응 확인 후 |
| 다국어 | 해당 없음 |
